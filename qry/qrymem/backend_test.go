...
var testQry = `(qry
	...
	infoLabel: (*prod.label _ id; label:('Label: ' .name))
...
func TestBackend(t *testing.T) {
	...
	tests := []struct {
		...
		{`(qry ?prod.label _ id; label:('Label: ' .name)))`, `{id:1 label:'Label: M'}`},
		...
		{`(qry ?prod.prod (eq .id 1) _ name; c:(?prod.cat (eq .id ..cat))))`,
			`{name:'A' c:{id:3 name:'c'}}`},
		{`(qry ?prod.prod (eq .id 1) _ name; cn:(?prod.cat (eq .id ..cat) _:name))`,
			`{name:'A' cn:'c'}`},
		{testQry, ``},
	}
