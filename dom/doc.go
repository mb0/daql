...
model versions are automatically determined in comparison to historic model information.  The
schemas and projects also have a numeric version derived in a similar fashion. The number 0 is used
if no history is provided. Otherwise we compare to the latest history version and use its version or
...
The dom package should itself have a JSON representations that can be sent to clients. Models can
